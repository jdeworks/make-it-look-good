...
> **Run the analyzer in multiple modes — never trust a single pass.** A design can pass at desktop/light yet fail at mobile or in dark mode (tiny touch targets, horizontal overflow, dark-mode contrast). In `analyzer.html`, set the **Viewport** dropdown (mobile 375 / tablet 768 / desktop 1280) and the **Color scheme** dropdown (Auto / Force light / Force dark), and **re-run** for a few targeted combinations — at minimum **mobile + desktop**, plus **light + dark** when the site/design supports dark mode. Turn on **Pixel Verify** for true rendered-contrast results, and **Explore SPA views** for apps where one tab can pass while another fails. Keep it to a handful of meaningful combos (a couple viewports × light/dark), not an exhaustive sweep. Apply the same multi-mode check to the design **you generate** in Step 3, not just the reference site.
