...
**This is not a library you install.** It's a design knowledge base (46 topics on color, typography, spacing, layout, interaction, accessibility) plus a consultation playbook that guides your AI through the right questions and decisions.
...
Give this repo to your AI assistant and it becomes a **design consultant** — asking the right questions, applying real design principles, and producing Tailwind/HTML-first UI or framework conversions that fit your stack. Not generic blue SaaS — your brand, your personality, your colors.
...
The bundle (~179k tokens) includes the full consultation playbook, all 46 knowledge files, and the template catalog (`docs/presets/index.json`). To keep it pasteable it does **not** embed the template HTML — fetch a specific preset on demand from `https://raw.githubusercontent.com/jdeworks/make-it-look-good/dev/docs/presets/<element>/<variant>.html` (variants per `index.json`), or browse them in the [live preview tool](https://jdeworks.github.io/make-it-look-good/) and tell your AI which ones you like.
...
```bash
./start.sh                 # convenience launcher (equivalent to: node server.js)
...
Run with `--offline` — the launcher vendors the CDN assets the first time (needs internet once), then serves with no network:
