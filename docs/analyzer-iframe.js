...
window.MilgIframe = (function() {
  ...
  // --- Screenshot settings (REVERT NOTES at bottom of this block) ---
  var SCREENSHOT_SCALE = 0.75;
  var SCREENSHOT_QUALITY = 0.8;       // WebP quality (reverted from PNG)
  var DOM_ZOOM = 2;                    // Zoom DOM 2x before capture for better text AA
  // Effective resolution = SCREENSHOT_SCALE * DOM_ZOOM = 1.5x
  // REVERT: set DOM_ZOOM = 1 to disable, or revert whole block to:
  //   SCREENSHOT_SCALE = 0.75; SCREENSHOT_QUALITY = 0.8; DOM_ZOOM = 1;
  ...
  function buildScreenshotScript(msgType) {
    var ss = { scale: SCREENSHOT_SCALE, quality: SCREENSHOT_QUALITY, zoom: DOM_ZOOM };
    return '(function(){' +
          ...
          's.onload=function(){' +
            ...
            'function _send(fullUri,maskUri){' +
              ...
              'parent.postMessage({type:"' + msgType + '",' +
                ...
                'screenshotMeta:{scale:_effectiveScale,viewportHeight:vh,sectionCount:1,' +
              ...
              '},"*")' +
            '}' +
            // DOM_ZOOM: scale up HTML for better text rendering before capture
            // REVERT: remove the zoom lines + change _effectiveScale back to _sc
            'if(_zoom>1){document.documentElement.style.zoom=_zoom;void document.body.offsetHeight;' +
              'console.log("[iframe-ss] DOM zoom applied: "+_zoom+"x")}' +
            // Step 1: Capture real screenshot FIRST (safe — no DOM modifications)
            '_prog("Capturing screenshot...");' +
            'console.log("[iframe-ss] Step 1/2: Capturing screenshot at "+_sc+"x (DOM "+_zoom+"x = "+_effectiveScale+"x effective)...");' +
            'ms.domToCanvas(document.documentElement,{scale:_sc,timeout:15000}).then(function(fc){' +
              ...
              'function _nextLayer(){' +
                ...
                'ms.domToCanvas(document.documentElement,{scale:_sc,timeout:12000}).then(function(mc){' +
                  ...
                  'layer.forEach(function(pe){' +
                    'if(!pe.bbox)return;' +
                    'var bx=Math.max(0,Math.round(pe.bbox.left*_effectiveScale));' +
                    'var by=Math.max(0,Math.round(pe.bbox.top*_effectiveScale));' +
                    'var bw=Math.min(Math.round(pe.bbox.width*_effectiveScale),mc.width-bx);' +
                    'var bh=Math.min(Math.round(pe.bbox.height*_effectiveScale),mc.height-by);' +
                    'if(bw<2||bh<2)return;' +
                    'try{var px=mCtx.getImageData(bx,by,bw,bh).data;' +
                      ...
                      'if(_dk===0&&bw>3&&pe.pair.text){' +
                        'try{var cs=getComputedStyle(pe.el);' +
                          ...
                          'var _fs=parseFloat(cs.fontSize)*_effectiveScale;' +
                          '_fx.fillStyle="#000";' +
                          ...
                          'var _pl=(parseFloat(cs.paddingLeft)||0)*_effectiveScale;' +
                          'var _pt=(parseFloat(cs.paddingTop)||0)*_effectiveScale;' +
                          'var _pr=(parseFloat(cs.paddingRight)||0)*_effectiveScale;' +
                          ...
                          'var _lh=parseFloat(cs.lineHeight)*_effectiveScale||_fs*1.2;' +
