...
window.MilgIframe = (function() {
  ...
  // Default 1× (fast/light, cleaner on AA small text); the analyzer UI (#spaCaptureScale) can
  // request 2× per run via opts.captureScale. Live override: localStorage.__milgSpaTune =
  ...
  function _analyzeHtmlInIframe(html, callback, sourceUrl, excludeSelector, captureScreenshots, viewportOverride, editorDark, editorEffectCSS, jsEnabled, forceBodyDarkUi) {
    ...
    function onMsg(e) {
      ...
      if (e.data.type === 'milg-analyzer-result') {
        ...
        // The fallback can fire after capture mutated the page (force-reveals, scroll),
        // so it would overwrite the correct data from the first extraction.
      ...
      if (e.data.type === 'milg-screenshots-result' && iframe._milgData) {
        ...
        // Hidden-panel content is covered by the region pipeline (per-panel
        // "Hidden section" screenshots) — no extra full-page unhidden pass.
  ...
  function analyzeSpaViews(html, opts, callback) {
    ...
    var spaOpts = {
      ...
      timeBudgetMs: opts.timeBudgetMs || (wantShots ? 90000 : 45000),
      ...
      maxStatePasses: opts.maxStatePasses != null ? opts.maxStatePasses : 8,
