...
window.MilgIframe = (function() {
  ...
  var SCREENSHOT_FORMAT = 'image/png'; // PNG = lossless, no compression artifacts on text colors
  ...
  function buildScreenshotScript(msgType) {
    var ss = { scale: SCREENSHOT_SCALE, format: SCREENSHOT_FORMAT };
    return '(function(){' +
        ...
        'setTimeout(function(){' +
          ...
          's.onload=function(){' +
            ...
            'ms.domToCanvas(document.documentElement,{scale:_sc,timeout:15000}).then(function(fc){' +
              ...
              'var fullUri;try{fullUri=fc.toDataURL("' + ss.format + '")}catch(e){fullUri=""}' +
    ...
    var unhiddenScreenshotFn = 'window.__milgDoUnhiddenScreenshots=function(){' +
      ...
      'setTimeout(function(){' +
        ...
        'ms.domToCanvas(document.documentElement,{scale:' + SCREENSHOT_SCALE + ',timeout:8000}).then(function(fc){' +
          'var uri;try{uri=fc.toDataURL("' + SCREENSHOT_FORMAT + '")}catch(e){uri=""}' +
