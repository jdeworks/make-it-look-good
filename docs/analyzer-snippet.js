  ...
  if (jsonSize > 4 * 1024 * 1024) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(json).then(function() {
        ...
        console.log('%c\u2139 Captured at this window\u2019s width. To analyze another viewport, pick it in the analyzer\u2019s viewport selector and re-run.', 'color: #6b7280; font-size: 12px;');
      }).catch(copyFallback);
    } else {
      copyFallback();
    }
