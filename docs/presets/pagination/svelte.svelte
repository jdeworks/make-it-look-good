...
<script>
  let { totalPages = 10, onPageChange } = $props();
...
<nav class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 px-4 sm:px-6 py-3" aria-label="Pagination">
  <div class="flex items-center justify-between gap-1">
    ...
    <button
      ...
      class={`inline-flex items-center justify-center min-h-11 min-w-11 px-2 sm:px-3 py-2 text-sm font-medium rounded-lg ${
        currentPage === 1
      ...
      {#each pages as page, i}
        ...
        {:else}
          <button
            ...
            class={`min-h-11 min-w-11 px-3 py-2 text-sm font-medium rounded-lg focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none ${
              page === currentPage
