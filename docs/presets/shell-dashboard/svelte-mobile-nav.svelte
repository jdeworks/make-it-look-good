...
<script>
  ...
  function navigate(id) { activePage = id; closeSidebar(); }
</script>
...
<div class="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100">
    ...
    <aside class={`fixed inset-y-0 left-0 lg:static w-60 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex-col z-30 lg:flex${sidebarOpen ? ' flex' : ' hidden'}`}>
      ...
      <nav class="flex-1 overflow-y-auto p-4 space-y-1" aria-label="Main navigation">
        {#each navItems as item (item.id)}
          ...
          <a
            href="#"
            class={`flex min-h-11 items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none${
              isActive
  ...
  <nav
    class="fixed bottom-0 inset-x-0 z-40 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 safe-area-bottom lg:hidden"
