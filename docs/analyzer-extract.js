    ...
    try {
      ...
      for (var _ai = 0; _ai < allElements.length && _ai < 5000; _ai++) {
        ...
        if (parseFloat(_as.opacity) === 0 && (_hasTrans || _hasAnim)) {
          // Only count elements that look SCROLL-REVEAL parked: laid out but outside the visible
          // fold, or translate-parked off-axis. An opacity-0 element sitting fully in-viewport
          // with no transform is hover-reveal UI (Tailwind group-hover rows, tooltips) — its
          // opacity:0 is the normal unhovered state, not "waiting for an animation trigger", and
          // counting those flooded real apps with a bogus motion warning. Elements with NO layout
          // box at all are excluded too: they're hidden by a collapsed/virtualized container (the
          // hidden-panel analysis owns that case), not parked for a scroll animation — narratu's
          // row buttons inside virtual list items kept a residual count via the no-rect branch.
